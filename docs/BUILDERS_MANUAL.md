...
| **Last worked** | 2026-06-24 (c) — **demo-readiness pass**, all deployed (localhost + GitHub + Vercel in sync at `dca97f2`): deterministic seeded batch (STP stable ≈81.6%); **Approve & Post** now actually posts → new `posted`/`rejected` lifecycle states (STP integrity kept — human-approved ≠ touchless); **USD currency** everywhere (`src/utils/currency.js`, was ₹/INR); **Eval & STP page rebuilt** (`EvalDashboard.jsx`) — guard-railed STP scorecard + real batch outcome + reviewer impact + doc-type mix + **AI usage & cost** panel (modeled Haiku-4.5 pricing). Demo scripts produced as Claude artifacts (see memory [[demo-state]]). |
| **Prev** | 2026-06-24 (b) doc-type variety (PO Invoice/Subscription/Non-PO + new Purchase Order & Credit Note reference docs + Type filter) · (a) real generated source PDFs + samples fully inspectable · 2026-06-23 (Phase E flywheel) |
| **Next action** | **RESUME: demo prep (planned for the day after 2026-06-24).** Pre-flight before presenting: (1) ⚠️ flip OFF Vercel **Deployment Protection** or the live URL 401s — only Natasha can; (2) decide demo target (localhost vs live URL); (3) optional polish not yet done — hide Approve/Reject on PO/Credit reference docs, fix the pre-existing `mapping.js:192` lint error. Demo collateral: 10-min run-of-show + 3-min narration artifacts (links in memory [[demo-state]]). After the demo, the standing §9 moves remain: real extraction + click-to-source, one real ERP post, a backend. |
