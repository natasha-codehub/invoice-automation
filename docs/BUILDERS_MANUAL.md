...
### Phase F — Eval & guardrail metrics (the original "Phase 5") ✓ (2026-06-25, browser-verified)
...
- [x] F1. Extraction-accuracy eval vs `groundTruth` + confidence calibration. `src/pipeline/evalMetrics.js` (`extractionAccuracy`, `calibration`) replays the **exact** live normalisation (`normalise` now exported from `validationEngine.js`) over the 4 golden invoices: raw field accuracy **87.5% → 100% after normalisation** (Vern Lewis ALL-CAPS vendor + Haun-Albany spaced PO are the 2 raw misses, both repaired). `EvalDashboard.ExtractionAccuracy` renders the raw→norm headline, a per-field raw/normalised/ground-truth table, and a calibration table (0.50–0.69 → 50% acc · 0.70–0.84 → 100% · 0.85–1.00 → 100%) — the engine is least accurate exactly where it flags low confidence. Per-engine framing kept (Demo today; swap Layer-1 → table re-runs on the same golden set).
- [x] F2. Guardrail board: STP never alone. `evalMetrics.guardrails(invoices, decisions)` computes all three from the live batch — `$-leakage` ($ value of touchless invoices carrying a blocking issue → $0), `exception-escape` (% of touchless that slipped a blocking issue → 0%), `false-reject` (rejections w/o a logged fatal reason → 0%). `Scorecard` now takes computed `gr` (was two hard-coded 0s) and flips each chip ✓ WITHIN / ⚠ CHECK on its own ok-state, so a regression would surface.
- [x] F3. Auto-approval controls. `evalMetrics.qaSample` pulls a **deterministic** 5% of auto-approved invoices for random-QA (FNV-1a hash sort, so it doesn't wobble on reload); `evalMetrics.shadowVendors` flags every invoice whose normalised vendor is **not** on the approved master (Airgas/Linde/Praxair/Norco/nexAir/Roberts/WestAir in the synthetic fill → ~571 invoices, 8 vendors, in shadow). `EvalDashboard.AutoApprovalControls` renders both as a two-column panel.

**Done when:** ✓ VERIFIED 2026-06-25 (browser, Eval slide-over, no console errors) — guardrail board reads STP 81.6% + $-leakage $0 + exception-escape 0% + false-reject 0%; extraction-accuracy shows 87.5%→100% with the per-field table (Vern vendor raw ✕ → norm ✓) and the 50/100/100 calibration bins; auto-approval controls show 41 pulled for QA ($163,957.49 sampled) and 8 new vendors / 571 invoices in shadow.
